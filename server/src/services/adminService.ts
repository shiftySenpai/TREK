...
export function createUser(data: { username: string; email: string; password: string; role?: string }) {
  ...
  const existingUsername = db
    .prepare('SELECT id FROM users WHERE username = ? AND COALESCE(is_guest, 0) = 0')
    .get(username);
...
export function updateUser(id: string, data: { username?: string; email?: string; role?: string; password?: string }) {
  ...
  if (username && username !== user.username) {
    const conflict = db
      .prepare('SELECT id FROM users WHERE username = ? AND id != ? AND COALESCE(is_guest, 0) = 0')
      .get(username, id);
    if (conflict) return { error: 'Username already taken', status: 409 };
  }
  if (email && email !== user.email) {
    const conflict = db
      .prepare('SELECT id FROM users WHERE email = ? AND id != ? AND COALESCE(is_guest, 0) = 0')
      .get(email, id);
...
export function createInvite(
  ...
  const ins = db
    .prepare('INSERT INTO invite_tokens (token, max_uses, expires_at, created_by, trip_id) VALUES (?, ?, ?, ?, ?)')
